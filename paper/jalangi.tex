...
& & \\
%     v1 = \dsl{new}(v2, v3,  \ldots) & \Longrightarrow & v2 = \Sync(v2)\\
% & & v3 = \Sync(v3)\\
% & & \vdots \\
% & & v1 = \Sync(\dsl{i\_new}(v2, v3, \ldots))\\
% & & \\
...
\texttt{trace[i] = v}.  However, the type of \texttt{v} could be an object or a
...
load of memory.  \emph{This is based on the observation that if we can
  ...
  value of the load.}  % In order to identify the memory loads that
% require no recording, we maintain a shadow memory during the recording
% phase, which gets updated only by the instrumented code.  During the
% recording phase, when we encounter memory loads with both shadow and
% actual values being the same, we skip recording the corresponding
% load. This significantly reduces the storage and communication
% overhead during the recording phase.
 
% In unoptimized record-replay, every value loaded from memory is
% recorded.  This could result in recording huge amount of data even for
% a small application.  In order to avoid recording of every memory
% load, \jalangi{} implements an optimized record-replay technique,
% where we record the value of a memory load only if the value cannot be
% derived solely by the replay of the instrumented code.
